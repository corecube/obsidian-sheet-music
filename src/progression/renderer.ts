...
export function registerProgressionPackage(plugin: Plugin): void {
	plugin.registerMarkdownCodeBlockProcessor("progression", (source, el) => {
		const model = parseProgression(source);
		if (!model) {
			el.createEl("p", {
				cls: "progression-error",
				text: "Add a key as the first line",
			});
			return;
		}
		...
		for (const row of model.rows) {
			...
			for (const item of row) {
				numeralRow
					.createEl("td")
					.createEl("em", { text: item.numeral });
				chordRow
					.createEl("td")
					.createEl("strong", { text: item.chord });
			}
		}
	});
}
